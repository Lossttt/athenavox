...
public class VoteCounter 
{
    ...
    protected static final int NUM_AGE_GROUPS = 4;     // Constant that defines the number of age groups in the election.
    ...
    private void calculateStatistics() 
    {
        ...
        calculateAgeGroupWinners();
    }

    void calculateAgeGroupWinners() 
    {
        ageGroupWinners = new int[NUM_AGE_GROUPS];

        for (int i = 0; i < NUM_AGE_GROUPS; i++) 
        {
            int startIndex = i * NUM_CANDIDATES;
            int endIndex = startIndex + NUM_CANDIDATES - 1;

            ageGroupWinners[i] = getTopCandidate(ageGroupVotes, startIndex, endIndex);
        }
    }
    ...
    public int[] getAgeGroupWinners() {
        return ageGroupWinners;
    }
